...
public class SceneLoader : MonoBehaviour {
  ...
  /// <summary>
  /// True to replace current scenes with new scenes. First scene loaded will
  /// become active scene.
  /// </summary>
  [Tooltip(
  ...
  /// <summary>
  /// True to automatically unload the loaded scenes when no more connections
  /// are using them.
  /// </summary>
  [Tooltip(
  ...
  /// <summary>
  /// True to fire when entering the trigger. False to fire when exiting the
  /// trigger.
  /// </summary>
  [Tooltip(
  ...
  /// <summary>
  /// Used to prevent excessive triggering when two clients are loaded and
  /// server is separate. Client may enter trigger intentionally then when moved
  /// to a new scene will re-enter trigger since original scene will still be
  /// loaded on server due to another client being in it. This scenario is
  /// extremely unlikely in production but keep it in mind.
  /// </summary>
  ...
  private void LoadScene(NetworkObject triggeringIdentity) {
    ...
    // NetworkObject isn't necessarily needed but to ensure its the player only
    // run if found.
    ...
    if (_moveAllObjects) {
      foreach (NetworkConnection item in InstanceFinder.ServerManager.Clients
                   .Values) {
    ...
    if (_connectionOnly)
      InstanceFinder.SceneManager.LoadConnectionScenes(triggeringIdentity.Owner,
                                                       sld);
