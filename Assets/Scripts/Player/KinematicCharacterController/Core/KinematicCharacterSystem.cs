...
/// <summary>
/// The system that manages the simulation of KinematicCharacterMotor and
/// PhysicsMover
/// </summary>
...
public class KinematicCharacterSystem : MonoBehaviour {
  ...
  public static List<KinematicCharacterMotor> CharacterMotors =
      new List<KinematicCharacterMotor>();
  ...
  /// <summary>
  /// Sets the maximum capacity of the character motors list, to prevent
  /// allocations when adding characters
  /// </summary>
  ...
  /// <summary>
  /// Sets the maximum capacity of the physics movers list, to prevent
  /// allocations when adding movers
  /// </summary>
  /// <param name="capacity"></param>
  ...
  // This is to prevent duplicating the singleton gameobject on script
  // recompiles
  ...
  /// <summary>
  /// Remembers the point to interpolate from for KinematicCharacterMotors and
  /// PhysicsMovers
  /// </summary>
  public static void PreSimulationInterpolationUpdate(float deltaTime) {
    ...
    for (int i = 0; i < CharacterMotors.Count; i++) {
      ...
      motor.Transform.SetPositionAndRotation(motor.TransientPosition,
    ...
    for (int i = 0; i < PhysicsMovers.Count; i++) {
      ...
      mover.Transform.SetPositionAndRotation(mover.TransientPosition,
...
#pragma warning restore 0162
  ...
  public static void PostSimulationInterpolationUpdate(float deltaTime) {
    ...
    for (int i = 0; i < CharacterMotors.Count; i++) {
      ...
      motor.Transform.SetPositionAndRotation(motor.InitialTickPosition,
  ...
  private static void CustomInterpolationUpdate() {
    float interpolationFactor =
        Mathf.Clamp01((Time.time - _lastCustomInterpolationStartTime) /
    ...
    for (int i = 0; i < CharacterMotors.Count; i++) {
      ...
      motor.Transform.SetPositionAndRotation(
          Vector3.Lerp(motor.InitialTickPosition, motor.TransientPosition,
                       interpolationFactor),
          Quaternion.Slerp(motor.InitialTickRotation, motor.TransientRotation,
                           interpolationFactor));
    }
    ...
    for (int i = 0; i < PhysicsMovers.Count; i++) {
      ...
      mover.Transform.SetPositionAndRotation(
          Vector3.Lerp(mover.InitialTickPosition, mover.TransientPosition,
                       interpolationFactor),
          Quaternion.Slerp(mover.InitialTickRotation, mover.TransientRotation,
                           interpolationFactor));
