...
public class ExamplePlayer : MonoBehaviour {
  ...
  private void LateUpdate() {
    ...
    if (CharacterCamera.RotateWithPhysicsMover &&
      ...
      CharacterCamera.PlanarDirection =
          Character.Motor.AttachedRigidbody.GetComponent<PhysicsMover>()
              .RotationDeltaFromInterpolation *
          CharacterCamera.PlanarDirection;
      CharacterCamera.PlanarDirection =
          Vector3
              .ProjectOnPlane(CharacterCamera.PlanarDirection,
  ...
  private void HandleCameraInput() {
    ...
    Vector3 lookInputVector =
        new Vector3(mouseLookAxisRight, mouseLookAxisUp, 0f);
    ...
    // Input for zooming the camera (disabled in WebGL because it can cause
    // problems)
    ...
    CharacterCamera.UpdateWithInput(Time.deltaTime, scrollInput,
                                    lookInputVector);
