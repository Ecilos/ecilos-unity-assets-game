...
public class FrameratePanel : MonoBehaviour {
  ...
  private void FixedUpdate() { _isFixedUpdateThisFrame = true; }
  ...
  void Update() {
    ...
    if (timeSinceLastPoll > PollingRate) {
      ...
      AvgFPS.text = GetNumberString(
          Mathf.RoundToInt(1f / (_framesDeltaSum / _framesCount)));
      AvgFPSMin.text =
          GetNumberString(Mathf.RoundToInt(1f / _maxDeltaTimeForAvg));
      AvgFPSMax.text =
          GetNumberString(Mathf.RoundToInt(1f / _minDeltaTimeForAvg));
    ...
    PhysicsRate.text =
        GetNumberString(Mathf.RoundToInt(1f / Time.fixedDeltaTime));
  }
...
public class FrameratePanelEditor : Editor {
  ...
  private void OnEnable() { InitStringsArray(); }
    ...
    for (int i = 0; i < fp.FramerateStrings.Length; i++) {
      if (i >= fp.FramerateStrings.Length - 1) {
        fp.FramerateStrings[i] = i.ToString() + "+" + " (<" +
                                 (1000f / (float)i).ToString("F") + "ms)";
      } else {
        fp.FramerateStrings[i] =
            i.ToString() + " (" + (1000f / (float)i).ToString("F") + "ms)";
      }
    }
  }
}
