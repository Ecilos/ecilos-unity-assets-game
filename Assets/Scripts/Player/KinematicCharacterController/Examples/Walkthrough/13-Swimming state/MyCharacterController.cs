...
public class MyCharacterController : MonoBehaviour, ICharacterController {
  ...
  /// <summary>
  /// This is called every frame by MyPlayer in order to tell the character what
  /// its inputs are
  /// </summary>
    ...
    // Handle detecting water surfaces
    {
      ...
      if (Motor.CharacterOverlap(Motor.TransientPosition,
        ...
        if (_probedColliders[0] != null) {
          // If the swimming reference point is inside the box, make sure we are
          // in swimming state
  ...
  public void UpdateRotation(ref Quaternion currentRotation, float deltaTime) {
    ...
    case CharacterState.Swimming: {
      ...
      if (OrientTowardsGravity) {
        ...
        currentRotation = Quaternion.FromToRotation(
                              (currentRotation * Vector3.up), -Gravity) *
                          currentRotation;
      }
      break;
    }
  ...
  public void UpdateVelocity(ref Vector3 currentVelocity, float deltaTime) {
    ...
    case CharacterState.Default: {
      ...
      if (Motor.GroundingStatus.IsStableOnGround) {
        ...
        Vector3 reorientedInput =
            Vector3.Cross(Motor.GroundingStatus.GroundNormal, inputRight)
                .normalized *
      ...
      // Handle jumping
      {
        ...
        if (_jumpRequested) {
          ...
          if (_canWallJump ||
            ...
            // snapped to the ground when trying to jump. Try commenting this
            // line out and see.
            Motor.ForceUnground(0.1f);
    ...
    case CharacterState.Swimming: {
      float verticalInput =
          0f + (_jumpInputIsHeld ? 1f : 0f) + (_crouchInputIsHeld ? -1f : 0f);
      ...
      // See if our swimming reference point would be out of water after the
      // movement from our velocity has been applied
      {
        ...
        Vector3 closestPointWaterSurface = Physics.ClosestPoint(
            resultingSwimmingReferancePosition, _waterZone,
        ...
        // project the velocity on the surface normal so that it would not take
        // us out of the water
        if (closestPointWaterSurface != resultingSwimmingReferancePosition) {
          Vector3 waterSurfaceNormal =
              (resultingSwimmingReferancePosition - closestPointWaterSurface)
                  .normalized;
          smoothedVelocity =
              Vector3.ProjectOnPlane(smoothedVelocity, waterSurfaceNormal);
  ...
  public void OnMovementHit(Collider hitCollider, Vector3 hitNormal,
    ...
    switch (CurrentCharacterState) {
    case CharacterState.Default: {
      // We can wall jump only if we are not stable on ground and are moving
      // against an obstruction
      if (AllowWallJump && !Motor.GroundingStatus.IsStableOnGround &&
