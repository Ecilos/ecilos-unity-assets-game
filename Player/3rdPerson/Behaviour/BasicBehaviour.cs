...
// This class manages which player behaviour is active or overriding, and call
// its local functions. Contains basic setup and common functions used by all
// the player behaviours.
public class BasicBehaviour : MonoBehaviour {
  public Transform
      playerCamera; // Reference to the camera that focus the player.
  public float turnSmoothing =
      0.06f; // Speed of turn when moving to match camera facing.
  public float sprintFOV =
      100f; // the FOV to use on the camera when player is sprinting.
  ...
  private int defaultBehaviour;  // The default behaviour of the player when any
                                 // other is not active.
  private int behaviourLocked;   // Reference to temporary locked behaviour that
                                 // forbids override.
  private Vector3 lastDirection; // Last direction the player was moving.
  private Animator anim;         // Reference to the Animator component.
  private ThirdPersonOrbitCamBasic
      camScript;           // Reference to the third person camera script.
  private bool sprint;     // Boolean to determine whether or not the player
                           // activated the sprint mode.
  private bool changedFOV; // Boolean to store when the sprint action has
                           // changed de camera FOV.
  ...
  private List<GenericBehaviour>
      behaviours; // The list containing all the enabled player behaviours.
  private List<GenericBehaviour>
      overridingBehaviours;   // List of current overriding behaviours.
  private Rigidbody rBody;    // Reference to the player's rigidbody.
  private int groundedBool;   // Animator variable related to whether or not the
  ...
  public ThirdPersonOrbitCamBasic GetCamScript {
  ...
  void FixedUpdate() {
    ...
    // Ensure the player will stand on ground if no behaviour is active or
    // overriding.
  ...
  // Check if any or a specific behaviour is currently overriding the active
  // one.
  ...
  // Use for temporary transitions like jumping, entering/exiting aiming mode,
  // etc.
  ...
  public bool IsHorizontalMoving() { return h != 0; }

  // Check if the player is moving.
  public bool IsMoving() { return (h != 0) || (v != 0); }
  ...
  public void Repositioning() {
    if (lastDirection != Vector3.zero) {
      ...
      Quaternion newRotation =
          Quaternion.Slerp(rBody.rotation, targetRotation, turnSmoothing);
  ...
  public bool IsGrounded() {
    Ray ray = new Ray(this.transform.position + Vector3.up * 2 * colExtents.x,
                      Vector3.down);
    return Physics.SphereCast(ray, colExtents.x, colExtents.x + 0.2f);
  }
}

// This is the base class for all player behaviours, any custom behaviour must
// inherit from this. Contains references to local components that may differ
// according to the behaviour itself.
public abstract class GenericBehaviour : MonoBehaviour {
  // protected Animator anim;                       // Reference to the Animator
  // component.
  protected int speedFloat; // Speed parameter on the Animator.
  protected BasicBehaviour
      behaviourManager;        // Reference to the basic behaviour manager.
  protected int behaviourCode; // The code that identifies a behaviour.
  protected bool canSprint;    // Boolean to store if the behaviour allows the
                               // player to sprint.
