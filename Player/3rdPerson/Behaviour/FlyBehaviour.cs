...
// FlyBehaviour inherits from GenericBehaviour. This class corresponds to the
// flying behaviour.
public class FlyBehaviour : GenericBehaviour {
  ...
  public float flyMaxVerticalAngle =
      60f; // Angle to clamp camera vertical movement when flying.

  private int flyBool;         // Animator variable related to flying.
  private bool fly = false;    // Boolean to determine whether or not the player
                               // activated fly mode.
  ...
  void Update() {
    // Toggle fly by input, only if there is no overriding state or temporary
    // transitions.
    if (Input.GetButtonDown(flyButton) && !behaviourManager.IsOverriding() &&
        !behaviourManager.GetTempLockStatus(
            behaviourManager.GetDefaultBehaviour)) {
      fly = !fly;

      // Force end jump transition.
      behaviourManager.UnlockTempBehaviour(
          behaviourManager.GetDefaultBehaviour);
      ...
      if (fly) {
        ...
        // Unregister this behaviour and set current behaviour to the default
        // one.
  ...
  public override void OnOverride() {
    // Ensure the collider will return to vertical position when behaviour is
    // overriden.
    col.direction = 1;
  }
