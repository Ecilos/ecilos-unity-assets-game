...
/// <summary>
/// Represents the entire state of a PhysicsMover that is pertinent for
...
public class PhysicsMover : MonoBehaviour {
  ...
  /// <summary>
  /// Determines if the platform moves with rigidbody.MovePosition (when true),
  /// or with rigidbody.position (when false)
  /// </summary>
  ...
  /// <summary>
  /// Returns all the state information of the mover that is pertinent for
  /// simulation
  /// </summary>
  ...
  public void VelocityUpdate(float deltaTime) {
    ...
    MoverController.UpdateMovement(out _internalTransientPosition,
                                   out _internalTransientRotation, deltaTime);
    ...
    if (deltaTime > 0f) {
      ...
      AngularVelocity =
          (Mathf.Deg2Rad * rotationFromCurrentToGoal.eulerAngles) / deltaTime;
    }
  }
}
}