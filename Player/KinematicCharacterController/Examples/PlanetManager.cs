...
public class PlanetManager : MonoBehaviour, IMoverController {
  ...
  private List<ExampleCharacterController> _characterControllersOnPlanet =
      new List<ExampleCharacterController>();
  ...
  public void UpdateMovement(out Vector3 goalPosition,
                             out Quaternion goalRotation, float deltaTime) {
    goalPosition = PlanetMover.Rigidbody.position;

    // Rotate
    Quaternion targetRotation =
        Quaternion.Euler(OrbitAxis * OrbitSpeed * deltaTime) * _lastRotation;
    ...
    foreach (ExampleCharacterController cc in _characterControllersOnPlanet) {
      cc.Gravity =
          (PlanetMover.transform.position - cc.transform.position).normalized *
          GravityStrength;
    }
  }
